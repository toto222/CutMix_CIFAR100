...
def parse_option():
    parser = argparse.ArgumentParser('ResNet34 for Classification')
    # Training setting
...
def main():
    ...
    save = os.path.join(args.save_dir, f'train_rn34_{args.pretrained}_{args.lr:.1e}')
    ...
    log_dir = './train_logs'
    writer = SummaryWriter(os.path.join(log_dir ,datetime.now().strftime("%m%d-%H%M")+f'_train_rn34_{args.lr:.1e}_{args.pretrained}'))
    
    model = timm.create_model('resnet34', pretrained=args.pretrained, num_classes=100).to(device)
